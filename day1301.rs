...
fn process_intcodes(intcodes: &mut Vec<i128>) {
    ...
    let mut memory: Vec<i128> = vec![0; 3_000];
    ...
    while i < intcodes.len() {
        ...
        match opcode {
            ...
            INPUT => {
                ...
                intcodes[index as usize] = SENSOR_BOOST;
                i += 2;
            }
            OUTPUT => {
                if all_params[0] == 1 {
                ...
                if output_count == 2 {
                    if intcodes[index as usize] == BLOCK_TILE {
                        block_tiles += 1;
                    }
                    output_count = 0;
                } else {
            ...
            STOP => {
                println!("Start the game. How many block tiles are on the screen when the game exits?\n{}", block_tiles);
